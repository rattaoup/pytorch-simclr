...
parser.add_argument("--lambda-to-run", type=str, default='1:2:3:4', help='lambda to run')
...
#Checking lambda index
lambda_to_run_str = args.lambda_to_run
lambda_to_run_list = lambda_to_run_str.split(':')
lambda_to_run_list = np.array([int(x) for x in lambda_to_run_list])
lambda_index = np.concatenate((lambda_to_run_list,lambda_to_run_list+4))-1
print('==> Running with lambda index .. {}'.format(lambda_to_run_list))
...
def train(epoch):
    ...
    for batch_idx, (inputs, _, _) in t:
        ...
        lambda_cut = torch.stack([brightness_list1, brightness_list2], dim=1)
        ...
        gradient_lambda =  autograd.grad(outputs = sum_rep,
                             ...
                             only_inputs = True)[0]
        #lambda to run
        gradient_lambda = gradient_lambda[:,lambda_index]

        # take norm before mean
        gradient_penalty = (gradient_lambda +1e-16).norm(p=args.norm, dim=1).clamp(max = 1000).mean()
...
for epoch in range(start_epoch, min(args.cut_off,last_epoch)):
