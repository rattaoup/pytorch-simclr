...
parser.add_argument("--method", type=str, default='uniform', help='method for random projection')
...
def train(epoch):
    ...
    for batch_idx, (inputs, _, _) in t:
        ...
        if(args.method == 'uniform'):
            coef1 = torch.empty(representation1.shape).uniform_(0,1).to(device)
            coef1 = coef1*(512*512/coef1.sum())
            sum_rep1 =  (coef1 * representation1).sum()

            coef2 = torch.empty(representation2.shape).uniform_(0,1).to(device)
            coef2 = coef2*(512*512/coef1.sum())
            sum_rep2 =  (coef2 * representation2).sum()
            sum_rep = sum_rep1 + sum_rep2
        if(args.method == 'zero_one'):
            coef1 = torch.empty(representation1.shape).random_(2).to(device)
            sum_rep1 =  (coef1 * representation1).sum()

            coef2 = torch.empty(representation2.shape).random_(2).to(device)
            sum_rep2 =  (coef2 * representation2).sum()
            sum_rep = sum_rep1 + sum_rep2
        if(args.method == 'minus_one'):
            coef1 = (2*torch.empty(representation1.shape).random_(2) - 1).to(device)
            sum_rep1 =  (coef1 * representation1).sum()

            coef2 = (2*torch.empty(representation2.shape).random_(2) - 1).to(device)
            sum_rep2 =  (coef2 * representation2).sum()
            sum_rep = sum_rep1 + sum_rep2
